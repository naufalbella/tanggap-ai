...
### 4. Deploy Frontend (Web UI)

Deploy the web frontend for easy access:
...
This deploys a modern web interface with:
...
TanggapAI includes a modern web interface for easy interaction with the system.

### Features

-   **📝 Feedback Analysis Form**: Submit customer feedback for instant analysis
-   **📊 Real-Time Results**: View analysis results immediately with color-coded displays
-   **📜 History Table**: Browse past analyses with filtering by sentiment and category
-   **🔍 Advanced Filters**: Filter feedback by sentiment (positive/neutral/negative) and category
-   **📱 Responsive Design**: Works seamlessly on desktop, tablet, and mobile devices
-   **🎨 Modern UI**: Clean, intuitive interface with gradient headers and card-based layout

### Screenshots

#### Analysis Form

Submit feedback and get instant analysis with sentiment, category, priority, keywords, root cause, and recommendations.

#### History Table

View all analyzed feedback with filters for sentiment and category. Table includes:
...
-   **Local Development**: Points to `http://localhost:8080` for backend API
-   **Production**: Uses same-origin requests (served from Cloud Run)

To customize the backend URL, edit `frontend/app.js`:
...
**Note:** Update `app.js` to point to your local backend:
...
# Run backend (on different port, e.g., 8000)
...
1. **Ollama Backend** - Model server (requires GPU)
2. **ADK Agent** - Analysis engine
3. **Backend API** - REST API and BigQuery integration
4. **Frontend** - Web interface

Each service's `deploy.sh` script automatically retrieves the required URLs from previous deployments.

## 🔄 Complete Deployment Script

Create a file `deploy-all.sh` in the root directory:

```bash
#!/bin/bash
set -e

echo "🚀 Deploying TanggapAI - Complete System"
echo "========================================"

# 1. Deploy Ollama Backend
echo ""
echo "📦 Step 1/4: Deploying Ollama Backend..."
cd ollama-backend
gcloud run deploy ollama-backend \
  --source . \
  --region europe-west1 \
...
# 2. Deploy ADK Agent
echo ""
echo "📦 Step 2/4: Deploying ADK Agent..."
cd adk-agent
export OLLAMA_URL=$(gcloud run services describe ollama-backend \
  --region=europe-west1 --format='value(status.url)')
gcloud run deploy tanggap-ai-adk-agent \
...
# 3. Deploy Backend API
echo ""
echo "📦 Step 3/4: Deploying Backend API..."
cd backend
./deploy.sh
cd ..
...
echo ""
echo "📍 Service URLs:"
echo "  Frontend:  $(gcloud run services describe tanggap-ai-frontend --region=europe-west1 --format='value(status.url)')"
echo "  Backend:   $(gcloud run services describe tanggap-ai-backend --region=europe-west1 --format='value(status.url)')"
echo "  Agent:     $(gcloud run services describe tanggap-ai-adk-agent --region=europe-west1 --format='value(status.url)')"
echo "  Ollama:    $(gcloud run services describe ollama-backend --region=europe-west1 --format='value(status.url)')"
echo ""
echo "🌐 Open Frontend:"
echo "  open $(gcloud run services describe tanggap-ai-frontend --region=europe-west1 --format='value(status.url)')"
```
